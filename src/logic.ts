...
export interface Options {
  ...
  pins?: (string | null)[];
  /** Per-player excluded spec keys ('Class:Name'), index-aligned with
   *  playerNames; empty array = nothing excluded for that player. Excluded
   *  specs can never be drawn for that player, including swap-in fixes. */
  exclusions?: string[][];
}
...
export function assignGroup(
  ...
  // Per-player exclusions: spec keys a given player must never be assigned.
  const excludedByPlayer = new Map<number, Set<string>>();
  (options.exclusions ?? []).forEach((keys, index) => {
    if (!keys || keys.length === 0) return;
    excludedByPlayer.set(index, new Set(keys));
  });
  ...
  const usedClasses = new Set<string>();  const candidates = (
    ...
    pools[role].filter((s) => {
      ...
      if (playerIndex !== undefined && excludedByPlayer.get(playerIndex)?.has(key(s))) {
        return false;
      }
      return true;
    });
  ...
  let dpsIndex = 0;
  const drawForRole = (role: Role, playerIndex?: number): Spec => {
    if (role !== 'DPS') {
      return drawFrom(candidates(role, false, undefined, undefined, playerIndex), role);
    }
    const req = dpsRequirements[dpsIndex++];
    return drawFrom(
      candidates('DPS', false, undefined, req, playerIndex),
      req ? `${req.toLowerCase()} DPS` : 'DPS',
    );
  };
  ...
  const assignments: Assignment[] = roles.map((role, i) => {
    ...
    return { playerName: names[i], role, spec: drawForRole(role, i) };
  });
  ...
  if (options.maxOneAug) {
    const extras = shuffle(
      assignments
        .map((a, idx) => ({ a, idx }))
        .filter(({ a }) => isAug(a.spec) && !pinnedAssignments.has(a)),
      rng,
    ).slice(1);
    for (const { a: target, idx } of extras) {
      usedSpecs.delete(key(target.spec));
      usedClasses.delete(target.spec.class);
      const pool = candidates(target.role, false, target, undefined, idx).filter((s) => !isAug(s));
      if (pool.length === 0) {
  ...
  if (wantBloodlust && lustCount() === 0) {
    ...
    const poolFor = (idx: number): Spec[] =>
      candidates(assignments[idx].role, true, assignments[idx], undefined, idx).filter(
        (s) =>
          !(
            options.maxOneAug &&
            isAug(s) &&
            assignments.some((x) => x !== assignments[idx] && isAug(x.spec))
          ) &&
          !(protectSoleBr && s.battleRez),
      );
    const swappable = assignments
      .map((a, idx) => ({ a, idx }))
      .filter(({ a, idx }) => !pinnedAssignments.has(a) && poolFor(idx).length > 0);
    if (swappable.length === 0) {
      if (!hasPins) throw new Error('Could not satisfy the bloodlust requirement');
    } else {
      const target = pick(swappable, rng);
      usedSpecs.delete(key(target.a.spec));
      usedClasses.delete(target.a.spec.class);
      target.a.spec = pick(poolFor(target.idx), rng);
      reserve(target.a.spec);
    }
  }
  ...
  if (options.mustIncludeBattleRez && brHolders().length === 0) {
    ...
    const poolFor = (idx: number): Spec[] =>
      candidates(assignments[idx].role, false, assignments[idx], undefined, idx).filter(
        (s) => s.battleRez,
      );
    const swappable = assignments
      .map((a, idx) => ({ a, idx }))
      .filter(
        ({ a, idx }) =>
          !(protectSoleLust && a.spec.bloodlust) && !pinnedAssignments.has(a) && poolFor(idx).length > 0,
      );
    if (swappable.length === 0) {
